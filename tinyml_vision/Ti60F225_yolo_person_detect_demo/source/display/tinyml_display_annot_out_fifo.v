...
module tinyml_display_annot_out_fifo (
output full_o,
output empty_o,
output [63:0] rdata,
input clk_i,
...
`IP_MODULE_NAME(tinyml_display_efx_fifo_top) #(
.OPTIONAL_FLAGS (0),
.SYNC_CLK (1),
.DEPTH (16),
.DATA_WIDTH (64),
...
module `IP_MODULE_NAME(tinyml_display_efx_fifo_datasync) #(
    parameter STAGE = 32,
    parameter WIDTH = 4
) (
 input  wire             clk_i,
...
module `IP_MODULE_NAME(tinyml_display_efx_fifo_gray2bin)
#(parameter WIDTH=5)
(// outputs
...
generate
   if (WIDTH > 1) begin
	 ...
	 `IP_MODULE_NAME(tinyml_display_efx_fifo_gray2bin) #(.WIDTH(WIDTH-1)) u_gray2bin (.bin_o(bin_o[WIDTH-2:0]), .gray_i({bin_1[0], gray_i[WIDTH-3:0]}));
      end
   end
   else /* if (WIDTH == 1) */
     assign bin_o = gray_i;
...
module `IP_MODULE_NAME(tinyml_display_efx_fifo_bin2gray)
#(parameter WIDTH=5)
(// outputs
...
module `IP_MODULE_NAME(tinyml_display_efx_fifo_top) # (
...
`IP_MODULE_NAME(tinyml_display_efx_fifo_ram) # (
    .FAMILY        (FAMILY),
    .MODE          (MODE),
    .WR_DEPTH      (WR_DEPTH),
    .RD_DEPTH      (RD_DEPTH),
...
`IP_MODULE_NAME(tinyml_display_efx_fifo_ctl) # (
    .FAMILY             (FAMILY),
    .SYNC_CLK           (SYNC_CLK),
...
module `IP_MODULE_NAME(tinyml_display_efx_fifo_ram) #(
...
module `IP_MODULE_NAME(tinyml_display_efx_fifo_ctl) # (
    parameter FAMILY             = "TRION",
...
generate
    if (SYNC_CLK) begin
        ...
        `IP_MODULE_NAME(tinyml_display_efx_fifo_bin2gray) # (.WIDTH(RADDR_WIDTH+1)                     ) xrd2wr_bin2gray    (.bin_i(raddr_cntr_w), .gray_o(raddr_cntr_gry));
        `IP_MODULE_NAME(tinyml_display_efx_fifo_datasync) # (.STAGE(SYNC_STAGE), .WIDTH (RADDR_WIDTH+1)) xrd2wr_addr_sync   (.clk_i(wclk), .d_i(raddr_cntr_gry_r), .d_o(raddr_cntr_gry_sync));
        `IP_MODULE_NAME(tinyml_display_efx_fifo_gray2bin) # (.WIDTH(RADDR_WIDTH+1)                     ) xrd2wr_gray2bin    (.gray_i(raddr_cntr_gry_sync), .bin_o(raddr_cntr_sync_g2b));
        ...
        `IP_MODULE_NAME(tinyml_display_efx_fifo_bin2gray) # (.WIDTH(WADDR_WIDTH+1)                     ) wr2rd_bin2gray  (.bin_i(waddr_cntr), .gray_o(waddr_cntr_gry));
        `IP_MODULE_NAME(tinyml_display_efx_fifo_datasync) # (.STAGE(SYNC_STAGE), .WIDTH (WADDR_WIDTH+1)) wr2rd_addr_sync (.clk_i(rclk), .d_i(waddr_cntr_gry_r), .d_o(waddr_cntr_gry_sync));
        `IP_MODULE_NAME(tinyml_display_efx_fifo_gray2bin) # (.WIDTH(WADDR_WIDTH+1)                     ) wr2rd_gray2bin  (.gray_i(waddr_cntr_gry_sync), .bin_o(waddr_cntr_sync_g2b));        
