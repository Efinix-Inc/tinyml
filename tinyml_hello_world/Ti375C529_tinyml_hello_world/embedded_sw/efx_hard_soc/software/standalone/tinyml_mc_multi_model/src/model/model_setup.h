...
bool setup_tflite_micro_model(
    ...
    for(int j = 0; j < model->input->dims->size; j++)
        MicroPrintf(" %d", model->input->dims->data[j]);
    MicroPrintf(", type: %d\n\r", model->input->type);
    ...
    for (int i = 0; i < model->total_output_layers; ++i) {
        ...
        for(int j = 0; j < model->interpreter->output(i)->dims->size; j++)
            MicroPrintf(" %d", model->interpreter->output(i)->dims->data[j]);
        MicroPrintf(", type: %d\n\r", model->interpreter->output(i)->type);
    }
