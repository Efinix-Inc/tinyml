...
inline void MulElementwise(int size, const ArithmeticParams& params,
  ...
  if(std::is_same<T, int8_t>::value) {
		if(OP_OK == mul_drv_elementwise(size,params.input1_offset, params.input2_offset, params.output_shift, params.output_multiplier,
										params.output_offset,params.quantized_activation_max, params.quantized_activation_min,
...
inline void BroadcastMul4DSlow(
	...
	if(std::is_same<T, int8_t>::value) {
		int bmax, ymax, xmax, cmax;
		bmax = extended_output_shape.Dims(0);
		ymax = extended_output_shape.Dims(1);
		xmax = extended_output_shape.Dims(2);
		cmax = extended_output_shape.Dims(3);
		if(OP_OK == mul_drv(params.input1_offset, params.input2_offset, params.output_shift, params.output_multiplier,
							params.output_offset, params.quantized_activation_max, params.quantized_activation_min,
							bmax, ymax, xmax, cmax, desc1.strides, desc2.strides, input1_data, input2_data, output_data)) {
			return;
